...
def get_enso_flavors(nino_indices, month_range=[12, 2],
    ...
    for y in times:
        time_range = [np.datetime64(f"{y}-{month_range[0]:02d}-01", "D"),
                      np.datetime64(f"{y+1}-{month_range[1]:02d}-01", "D")]
        # Select region
...
def get_enso_years(nino_indices, month_range=[12, 2],
    ...
    for season_type in ["Nino_EP", "Nino_CP", "Nina_CP", "Nina_EP"]:
    ...
    standard_years = np.array([
        standard_.loc[(standard_['type'] == 'standard')]['start'],
        standard_.loc[(standard_['type'] == 'standard')]['end'],
    ]).T

    return {'nino': nino_years,
            'nina': nina_years,
            'standard': standard_years,
            'Nino_CP': Nino_CP_years,
            'Nino_EP': Nino_EP_years,
            'Nina_CP': Nina_CP_years,
            'Nina_EP': Nino_EP_years,
            }
